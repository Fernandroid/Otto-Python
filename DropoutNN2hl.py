...
def DropoutNN2hl(nn_params,P,input_layer_size,hidden_layer_size,num_labels,X, label,landa):                                    
    ...
    m = X.shape[0]
    p1=P[0,0]; #p for input layer
    ph=P[0,1]; #p for hidden layer 
    J = 0.
    ...
    X1=X*np.random.binomial(1,p1,(m,input_layer_size)) 
    X1 = np.hstack([np.ones((m, 1)), X1])
    z2=np.dot(X1,Theta1.T)
    a2=sigmoid(z2)
    a2_h=a2*np.random.binomial(1,ph,(m,hidden_layer_size))  
    a2_h = np.hstack([np.ones((m, 1)), a2_h])
    z3=np.dot(a2_h,Theta2.T)
    h=SoftMax(z3.T)
    ...
    Theta2_grad=np.dot(delta3.T,a2_h)/m
    Theta1_grad=np.dot(delta2.T,X1)/m
    b2=Theta2_grad[:,0]
    b1=Theta1_grad[:,0]
